<script lang="ts">
  ...
  import { facepaintCanvasFileName, facepaintTexFileName } from '../shareMii/ugcKinds';
  import {
    getSidecarStore,
    hasOriginal,
    replaceSidecarFiles,
  ...
  async function loadCurrentPreview(id: number): Promise<void> {
    ...
    const codec = await import('../ugcEditor/codec');
  ...
  async function applyReplace(): Promise<void> {
    ...
    busy = true;
    try {
      const { encodeFromRgba } = await import('../ugcEditor/codec');
      const id = selectedId;
  ...
  async function exportSelectedAsPng(): Promise<void> {
    ...
    try {
      ...
      const { decodeZsFile, rgbaToPngBlob } = await import('../ugcEditor/codec');
